...
class Job {
    ...
    async reName(name, newName){
        try {
            let jobFolder = this.getJobFolder(name);
            let endpoint = this.jenkins.stringQuery(JOB_RENAME,jobFolder[0],jobFolder[1],newName);
            let rs = await this.jenkins.callAPI(endpoint,'POST','',true);
            return rs;
        } catch (error) {
            throw new Error('Error at rename job');
        }
    }
