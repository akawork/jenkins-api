...
class Job {
    ...
    async copy(fromjob,tojob){
        ...
        let is_exist = await this.is_exist(tojob)
        if(is_exist){
     ...
     */
    async enable(name){
        let is_exist = await this.is_exist(name)
        if(!is_exist){
            throw new Error('This job is not exist')
        }
        let [folder_job,short_name] = this.get_job_folder(name);
        let endpoint = this.jenkins.stringQuery(ENABLE_JOB,folder_job,short_name);
        await this.jenkins.callAPI(endpoint,'POST','',true);
    }
    ...
    async disable(name){
        ...
        let [folder_job,short_name] = this.get_job_folder(name);
        let endpoint = this.jenkins.stringQuery(DISABLE_JOB,folder_job,short_name);
        await this.jenkins.callAPI(endpoint,'POST','',true);
    }
    ...
    async delete(name){
        ...
        let [folder_job,short_name] = this.get_job_folder(name);
        let endpoint = this.jenkins.stringQuery(DELETE_JOB,folder_job,short_name);
        await this.jenkins.callAPI(endpoint,'POST','',true);
    }
    ...
    async reconfig(name,config){
        try {
            let [folder_job,short_name] = this.get_job_folder(name);
            let endpoint = this.jenkins.stringQuery(CONFIG_JOB,folder_job,short_name);
